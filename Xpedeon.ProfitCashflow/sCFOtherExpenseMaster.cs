...
namespace Xpedeon.ProfitCashflow
{
    class sCFOtherExpenseMaster
    {
        ...
        public DataTable RetrieveMaster(decimal dCompanyId)
        {
            ...
            using (SqlDataAdapter da = new SqlDataAdapter())
            {
                da.SelectCommand = new SqlCommand("SPN_XP_GET_CF_EXPENSE", ProfitCashflow.oPcfDM.conn);
                da.SelectCommand.CommandType = CommandType.StoredProcedure;
        ...
        {
            int iResult;
            using (SqlCommand cmd = new SqlCommand("SPN_XP_CHECK_CF_EXPENSE", ProfitCashflow.oPcfDM.conn))
            {
        ...
        {
            int iResult;
            using (SqlCommand cmd = new SqlCommand("SPN_XP_MAX_CF_EXPENSE_ID", ProfitCashflow.oPcfDM.conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@IN_COMPANY_ID", SqlDbType.Decimal).Value = dCompanyId;
        ...
        public void UpdateMaster(string sType, DataRow drMas)
        {
            ...
            using (TransactionScope ts = new TransactionScope(TransactionScopeOption.RequiresNew, to, EnterpriseServicesInteropOption.Full))
            {
                ...
                {
                    if (sType == "I")
                        cmd = new SqlCommand("SPN_XP_INS_CF_EXPENSE_MASTER", ProfitCashflow.oPcfDM.conn);
                    else if (sType == "U")
                        cmd = new SqlCommand("SPN_XP_UPD_CF_EXPENSE_MASTER", ProfitCashflow.oPcfDM.conn);
                    else if (sType == "D")
                    {
                        cmd = new SqlCommand("SPN_XP_DEL_CF_EXPENSE_MASTER", ProfitCashflow.oPcfDM.conn);
